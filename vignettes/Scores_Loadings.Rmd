...
vignette: >
    %\VignetteIndexEntry{LearnPCA 5: Understanding Scores and Loadings}
...
kable(df, format = "html", caption = "Individual values for the variables and samples.") %>% kable_styling(c("striped", "bordered"), full_width = FALSE)
var2 = var(variable2)
var1 = var(variable1)
...
Variable 2 accounts for `r round(percent_var2, digits = 1)`% of the total variance and variable 1 accounts for the remaining `r round(percent_var1, digits = 1)`% of the total variance.
...
For a simple system with just two variables, this process amounts to a simple rotation of the two axes. Suppose we rotate the axes in Figure \@ref(fig:fig-origData) by 20° and project the data points onto the new axes. Figure \@ref(fig:rotate20) shows the result where the dashed lines are the original axes associated with variable 1 and variable 2, and the solid lines are the rotated axes, which we identify for now as axis A and axis B to clarify that they are not specifically associated with one of the two variables, nor with the principal components we eventually hope to find. It is easy to see---both visually and quantitatively---that the total variance is more evenly distributed between the two rotated axes than was the case for the original two axes.

```{r rotate20, fig.align="center", fig.cap="Rotating the original axes by 20° gives the axes shown as solid lines. The small points in pink and in blue are the projections of the original data onto the rotated axes. The pink and blue dotted lines illustrate the projections for one data point."}
...
If we continue to rotate the axes, we discover that an angle of rotation of 60° yields an axis that explains almost 99% of the total variance in our eight samples. We call this the first principal component axis; the remaining axis, which is perpendicular to the first principal component axis, is the second principal component axis.

```{r rotate60, fig.align="center", fig.cap="Rotating the original axes by 60° maximizes the variance along one of the two rotated axes; the line in pink is the first principal component axis and the line in blue is the second principal component axis."}
...
In Figure \@ref(fig:fig-origData), each or our eight samples appears as a point in space defined by its position along the axes for variable 1 and variable 2. After completing the rotation of the axes, each of the eight samples now appears as a point in space defined by its position along the two principal component axes. These positions are the scores returned by a PCA analysis. Table \@ref(tab:table-scores) provides the scores for our eight samples in the columns labeled PC 1 and PC 2; also shown are the values for variable 1 and variable 2.
...
kable(df, format = "html", caption = "Individual results for each of the eight samples.") %>% kable_styling(c("striped", "bordered"), full_width = FALSE)
```

## Loadings

Although the scores define the location of our samples relative to the principal component axes, they do not define the location of the principal component axes in space. To reconstruct the original data from the results of a PCA analysis, as described in the vignette [Step-by-Step PCA](#top-matter) we must know both where the samples lie relative to the principal component axes **and** the location of the principal component axes relative to the original axes. As shown in Figure \@ref(fig:rotate60-loadings), each principal component axis is defined by the cosine of its angle of rotation relative to each of the original axes. For example, the angle of rotation of PC 1 to the axis for variable 1, identified here as $\Theta$, is --60°, which gives its loading as $\cos(-60) = 0.500$. The angle of rotation of PC 1 to the axis for variable 2, or $\Phi$, returns a loading of $\cos(30) = 0.866$.
...
Given its cosine function, individual loadings are limited to the range --1 (an angle of rotation of 180°) to +1 (an angle of rotation of 0°). A positive loading indicates that a variable contributes in some way to the principal component and a negative loading indicates that its absence contributes in some way to the principal component. The larger a loading's relative magnitude, the more important is its presence or absence to the principal component.
