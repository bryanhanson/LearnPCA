...
Here is a function that will compute PCA using the NIPALS algorithm:
...
simpleNIPALS <- function(X, pcs = 3, tol = 1e-6) {
  ...
  for (pc in 1:pcs) { # compute one PC at a time
    ...
    while (!done) { # Note 2
      l <- t(X) %*% s_old # compute/improve loading estimate by projecting scores onto raw data
      l <- l/sqrt(sum(l^2)) # scale by l2 norm
      s_new <-X %*% l # update estimate of scores
      err <- sqrt(sum((s_new - s_old)^2)) # compute the error of the score estimate
      if (err < tol) {done <- TRUE; next} # check error against tolerance, exit if good
      s_old <- s_new # if continuing, use the best score estimate
    }
    X <- X - s_new %*% t(l) # "deflate" the data by removing the contribution of the results so far Note 3
