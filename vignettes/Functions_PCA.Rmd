...
vignette: >
    %\VignetteIndexEntry{LearnPCA 6: Functions for PCA}
...
`prcomp` is probably the function most people will use for PCA, as it will handle input data sets of arbitrary dimensions (meaning, the number of observations $n$ may be greater or less than the number of measured variables, $p$). `prcomp` can do centering or scaling for you, but it also recognizes when the data passed to it has been previously centered or scaled via the `scale` function.[^2] It turns out that `prcomp` is a wrapper for the `svd` function (which we'll discuss below).  This means that `prcomp` uses `svd` "under the hood" and repackages the results from `svd` in a more useful form.

# `princomp`

In contrast to `prcomp`, `princomp` cannot handle the situation where $p > n$, so it is not as flexible.  In addition, `princomp` uses function `eigen` under the hood, and `eigen` only accepts square matrices as input (where $n = p$).  `princomp` will accept a rectangular matrix with $n > p$ however because along the way, `princomp` will convert the input matrix to either the covariance or correlation matrix before passing the new matrix to `eigen` (the choice depends upon the arguments).  Both the covariance and correlation matrices are square matrices that `eigen` can process.
...
If you center the columns of $A$, then take $A^TA$ and divide by $n -1$ you get the **covariance** matrix. We can verify this via the built-in function `cov`. Covariance is unbounded.
...
If instead you center the columns of $A$, autoscale[^5] the columns, and then take $A^TA$ and divide by $n -1$ you get the **correlation** matrix. Correlation is covariance normalized and is bounded on $[-1, 1]$.  Again we can verify this with the built-in function `cor`:
...
Notice that the absolute values of the scores are similar, but not identical.  This is because different algorithms were used in each case.  Notice too that the pattern of positive and negative values is complex, it's not as if one is simply the negative of the other.  **TODO elaborate**
...
The absolute value of the loadings from each method are identical, while the actual pattern of positive and negative values again is complex.
...
c2 = c("uses svd", "$n \\times p$", "center; scale", "center; scale[^4]", "$pca \\leftarrow prcomp(X)$", "$pca\\$x$", "$pca\\$rotation$", "$\\cfrac{100*pca\\$sdev^2}{sum(pca\\$sdev^2)}$")
...
kable(DF, format = "html", row.names = FALSE, caption = "A comparison of R functions for PCA.  Input data must be centered in all cases.  Scaling is optional.") %>% kable_styling(c("striped", "bordered"), full_width = FALSE)
```
...
[^4]: Obviously one should either center and scale ahead of time, or allow `prcomp` to do it, not both.
[^5]: Autoscaling means to divide each column by its standard deviation.
