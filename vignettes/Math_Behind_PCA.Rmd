...
The SVD decomposition breaks the original data matrix $\mathbf{X}$ into three matrices (dimensions in parentheses):[^5]
...
* The columns of $\mathbf{U}$ will contain the PCA scores
...
* The columns of $\mathbf{V}$ will contain the PCA loadings
...
And now we'll do a simple iterative calculation that computes the values of `U` and `V`.[^2]  The "for" loop runs for 50 iterations, and as it does the values of `U` and `V` are continuously updated (and get closer to the actual answer).
...
The Eigen decomposition is another way to decompose a data matrix. This decomposition breaks the data matrix $\mathbf{X}$ into two matrices:[^8]

\begin{equation}
(\#eq:ed1)
\mathbf{X}_{(n \ \times \ n)} = \mathbf{Q}_{(n \ \times \ n)}\mathbf{\Lambda}_{(n \ \times \ n)}\mathbf{Q}_{(n \ \times \ n)}^{-1}
\end{equation}
...
The equation above is where we'd like to end up.  How can we get there?  Once again, let's take inventory of the matrices in the equation.

* $\mathbf{X}$ is the original data matrix.  Notice that unlike in SVD the dimensions are $n \times n$.  In other words, it is a square matrix.  Your data is not square you say?  Eigen decomposition requires an square matrix.  Fortunately, there's a simple fix for this.  We can work instead with the covariance matrix, which is square: $\mathbf{X}^T\mathbf{X}/(n - 1)$. This retains all the information needed for the computation.  In addition, we should center the data before proceeding.
* $\mathbf{Q}$ is square matrix that whose columns will contain the *eigenvectors*.  $\mathbf{Q}$ is also an orthonormal matrix (discussed earlier in the SVD section).  Notice that $\mathbf{Q}$ appears twice in \@ref(eq:ed2), the second time as its inverse: $\mathbf{Q}^{-1}$.
* $\mathbf{\Lambda}$ (upper case Greek Lambda) is a diagonal matrix, very similar to $\mathbf{D}$ in SVD. All non-diagonal elements are zero.  The diagonal contains values sorted from largest to smallest.  These are called the *eigenvalues*.

So what are eigenvectors and eigenvalues?  If you square the eigenvalues, you have the singular values from SVD. Here "diagonal" means take the diagonal elements of the matrix (which would a vector of values):[^10]

\begin{equation}
(\#eq:ed3)
diagonal(\mathbf{D}) = (diagonal(\mathbf{\Lambda}))^2
\end{equation}

The eigenvectors on the other hand are called the principal axes, if we multiply the original data by the eigenvectors in $Q$ we get the scores:

\begin{equation}
(\#eq:ed4)
\mathbf{Q}\mathbf{X} = scores
\end{equation}
...
[^9]: We talk about the singular values here for completeness, but we won't make any use of them, as it turns out.
[^10]: As with SVD, there isn't any useful interpretation of $\mathbf{\Lambda}$ in terms of the data.
