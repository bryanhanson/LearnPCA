...
* scores matrix with $n$ rows and $p$ columns, where each column corresponds to a principal component and the values are the scores, namely the positions of the samples in the new reference frame.
...
* The columns of $\mathbf{U}$ are vectors giving the principal axes. These define the new frame of reference
...
Either of these "values" can be used to compute the amount of variance explained by each principal component.  Details are in the [PCA Functions](http://bryanhanson.github.io/LearnPCA/articles/Functions_PCA.html) vignette.
