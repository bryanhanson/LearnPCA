...
output:
    bookdown::html_document2:
      ...
      css: vignette.css
vignette: >
    %\VignetteIndexEntry{LearnPCA 03: Step-by-Step PCA}
...
kable(FeCu2, format = "html", row.names = FALSE, caption = "The FeCu data set. Values for Fe~2~O~3~ are percentages, those for Cu are ppm.") %>% kable_styling(c("striped", "bordered"), full_width = FALSE)
```
...
p <- ggplot(data = Tib, aes(x = name, y = value)) +
  geom_point() +
  labs(x = "", y = TeX(r"(Values ($Fe_{2}O_{3}$ in percent, Cu in ppm))"))
p
```
...
p <- ggplot(data = FeCu, aes(x = Fe2O3, y = Cu)) +
  geom_point() +
  labs(x = TeX(r"($Fe_{2}O_{3}$ (percent))"), y = "Cu (ppm)")
p
```
...
Please both the [The Math Behind PCA](#top-matter) and [Understanding Scores & Loadings](#top-matter) vignettes for a full discussion of the details of the calculation.
