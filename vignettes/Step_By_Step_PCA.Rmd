...
TO <- tintoodiel$tab # for convenience, rename the element with the data
...
kable(FeCu, format = "html", row.names = FALSE, caption = "The FeCu data set. Values for Fe2O3 are percentages, those for Cu are ppm.") %>% kable_styling(c("striped", "bordered"), full_width = FALSE)
```

```{r plot-raw-dotplot, eval = TRUE, fig.cap = "The range of the raw data values in `FeCu`."}
Tib <- pivot_longer(FeCu, everything())
p <- ggplot(data = Tib, aes(x = name, y = value)) +
  geom_point() +
  labs(x = "", y = "Values (Fe2O3 in percent, Cu in ppm)")
p
```

```{r plot-raw-data-2D, eval = TRUE, fig.cap = "The relationship between the raw data values in `FeCu`."}

p <- ggplot(data = FeCu, aes(x = Fe2O3, y = Cu)) +
  geom_point() +
  labs(x = "Fe2O3 (percent)", y = "Cu (ppm)")
p
```
...
When we center the data, we take each column, corresponding to a particular variable, and subtract the mean of that column from each value in the column.  Thus, regardless of the original values in the column, the centered values are now expressed relative to the mean value for that column.  The function `scale` can do this for us; `scale` actually can center and/or scale the data:

```{r center-raw-data, echo = TRUE}
FeCu_centered <- scale(FeCu, scale = FALSE, center = TRUE) # see ?scale for defaults
```

Figure \@ref(fig:plot-centered-data) is a plot of the centered values. Note how the values on the y-axis have changed compared to the raw data.  It's apparent that the ranges of the chosen variables are quite different.  This is a classic case where scaling is desirable, otherwise the variable with higher absolute values will dominate the PCA results.

```{r plot-centered-data, eval = TRUE, fig.cap = "Centered data values in `FeCu`."}
Tib <- pivot_longer(as.data.frame(FeCu_centered), everything())
p <- ggplot(data = Tib, aes(x = name, y = value)) +
  geom_point() +
  labs(x = "", y = "centered values")
p
```
...
Tib <- pivot_longer(as.data.frame(FeCu_centered_scaled), everything())
p <- ggplot(data = Tib, aes(x = name, y = value)) +
  geom_point() +
  labs(x = "", y = "centered & scaled values")
p
```
...
Scores represent the original data but in a new abstract space.  Figure \@ref(fig:plot-scores) shows the scores.


```{r plot-scores, fig.cap = "Scores."}
p <- ggplot(data = as.data.frame(pca_FeCu$x), aes(x = PC1, y = PC2)) +
     geom_point()
p
```
...
sg_all <- data.frame(pca_TO$x)
p <- ggplot(data = sg_all, aes(x = PC1, y = PC2)) +
     geom_point()
p
```
